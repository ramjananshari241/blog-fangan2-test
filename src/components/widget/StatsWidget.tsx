...
/**
 * 商家定制版 StatsWidget
 * 模仿 ProfileWidget 逻辑，直接从 data 中提取 Notion 数据库属性
 */
export const StatsWidget = ({ data }: { data: any }) => {
  ...
  // 核心逻辑：模仿 Profile 读取属性的方式
  // 尝试多种可能的解析路径，确保 100% 捕获在 Notion "repost" 栏填写的链接
  const purchaseLink = data?.repost || data?.link || data?.url || data?.custom_link || '#';
              ...
              <button 
                onClick={() => {
                  if (purchaseLink && purchaseLink !== '#') {
                    ...
                    alert('检测到数据，但 repost 属性为空，请在 Notion 中填写完整 URL')
                  }
                }} 
                type="button" 
